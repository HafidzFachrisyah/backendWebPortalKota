...
<script type="text/javascript">
    ...
    xhr.open('POST', '<?= base_url().SITE_URL ?>page/upload');
    ...
    xhr.send(formData);
		...
		tinymce.init({
          ...
          toolbar: 'undo redo | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image ',
	        images_upload_url: '<?= base_url().SITE_URL ?>page/upload',
...
<div class="row">
        <div class="col-12">
          <form name="f_page" id="f_page" method="POST" enctype="multipart/form-data">
          <div class="card mb-4">
            <div class="card-header pb-0">
              ...
              <div class="row">
                <div class="col col-10 text-left">
                  <a href="<?= base_url().SITE_URL?>page"><i class="fa fa-arrow-left me-2"></i> Back</a>
                </div>
                <div class="col col-2 text-right clearfix">
                  <div class="input-group text-right clearfix">
                      <button type="submit" id="btn_f_page" class="btn btn-info float-right"><i class="fa fa-save"></i> <?= ($id)?'Update':'Save'?></button>
                    </div>
                </div>
              </div>
                  ...
                  <hr>
                        <label class="form-label">Danger Zone</label>
                          <div class="d-grid gap-2">
                            <button type="button" class="btn btn-outline-danger" onclick="confirmDeletePage()"><i class="fa fa-trash"></i> Delete</button>
                          </div>
                  <?php }?>
                  </div>
      ...
      <script>
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              $.ajax({
                    url: "<?= base_url().SITE_URL?>page/delete",
                    type: "POST",
                    data: {
                        id: $("#id").val()
                    },
                    ...
                    success: function (data) {
                      ...
                      if(response.status==200){
                        Swal.fire({
                          ...
                          closeOnConfirm: true,
                          willClose: () => {
                            window.location.replace("<?= base_url().SITE_URL ?>page");
                          }
                        });
                      } else {
...
<script>
    $(document).ready(function() {
			...
			$('#f_page').on('submit', function(e) {
				e.preventDefault();
        $("#btn_f_page").prop('disabled', true);
        $("#btn_f_page").html('Saving..');
        var data = new FormData();
        data.append('data', jQuery('#f_page').serialize());
        data.append('content', tinyMCE.get('tinymce').getContent());

        jQuery.ajax({
          url: '<?= base_url().SITE_URL?>page/store',
          data: data,
          cache: false,
          contentType: false,
          processData: false,
          ...
          success: function(data){
            var response = JSON.parse(data);
            if(response.status==200){
              $("#btn_f_page").prop('disabled', false);
              $("#btn_f_page").html('<i class="fa fa-save"></i> Update');
              $.toast({
                  heading: 'Success',
                  text: "Success, New page created.",
                ...
                if ($('#dangerzone').children().length < 1){
                   $('#dangerzone').append('<hr><label class="form-label">Danger Zone</label><div class="d-grid gap-2"><button type="button" class="btn btn-outline-danger" onclick="confirmDeletePage()"><i class="fa fa-trash"></i> Delete</button></div>');
                }
            } else {
                $("#btn_f_page").prop('disabled', false);
                $("#btn_f_page").html('<i class="fa fa-save"></i> Save');
                $.toast({
                  heading: 'Error',
