...
<script type="text/javascript">
    ...
    xhr.open('POST', '<?= base_url().SITE_URL ?>banner/upload');
    ...
    xhr.send(formData);
		...
		tinymce.init({
          ...
          toolbar: 'undo redo | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image ',
	        images_upload_url: '<?= base_url().SITE_URL ?>banner/upload',
...
<div class="row">
        <div class="col-12">
          <form name="f_banner" id="f_banner" method="POST" enctype="multipart/form-data">
          <div class="card mb-4">
            <div class="card-header pb-0">
              ...
              <div class="row">
                <div class="col col-10 text-left">
                  <a href="<?= base_url().SITE_URL?>banner"><i class="fa fa-arrow-left me-2"></i> Back</a>
                </div>
                <div class="col col-2 text-right clearfix">
                  <div class="input-group text-right clearfix">
                      <button type="submit" id="btn_f_banner" class="btn btn-info float-right"><i class="fa fa-save"></i> <?= ($id)?'Update':'Save'?></button>
                    </div>
                </div>
              </div>
                  ...
                  <hr>
                        <label class="form-label">Danger Zone</label>
                          <div class="d-grid gap-2">
                            <button type="button" class="btn btn-outline-danger" onclick="confirmDeleteBanner()"><i class="fa fa-trash"></i> Delete</button>
                          </div>
                  <?php }?>
                  </div>
      ...
      <script>
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              $.ajax({
                    url: "<?= base_url().SITE_URL?>banner/delete",
                    type: "POST",
                    data: {
                        id: $("#id").val()
                    },
                    ...
                    success: function (data) {
                      ...
                      if(response.status==200){
                        Swal.fire({
                          ...
                          closeOnConfirm: true,
                          willClose: () => {
                            window.location.replace("<?= base_url().SITE_URL ?>banner");
                          }
                        });
                      } else {
...
<script>
    $(document).ready(function() {
			...
			$('#f_banner').on('submit', function(e) {
				e.preventDefault();
        $("#btn_f_banner").prop('disabled', true);
        $("#btn_f_banner").html('Saving..');
        var data = new FormData();
        jQuery.each(jQuery('#cover')[0].files, function(i, file) {
            data.append('cover-'+i, file);
        });

          data.append('data', jQuery('#f_banner').serialize());

        jQuery.ajax({
          url: '<?= base_url().SITE_URL?>banner/store',
          data: data,
          cache: false,
          contentType: false,
          processData: false,
          ...
          success: function(data){
            var response = JSON.parse(data);
            if(response.status==200){
              $("#btn_f_banner").prop('disabled', false);
              $("#btn_f_banner").html('<i class="fa fa-save"></i> Update');
              $.toast({
                  heading: 'Success',
                  text: "Success, New banner created.",
                ...
                if ($('#dangerzone').children().length < 1){
                   $('#dangerzone').append('<hr><label class="form-label">Danger Zone</label><div class="d-grid gap-2"><button type="button" class="btn btn-outline-danger" onclick="confirmDeleteBanner()"><i class="fa fa-trash"></i> Delete</button></div>');
                }
            } else {
                $("#btn_f_banner").prop('disabled', false);
                $("#btn_f_banner").html('<i class="fa fa-save"></i> Save');
                $.toast({
                  heading: 'Error',
