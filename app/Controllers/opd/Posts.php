...
class Posts extends BaseController
{
	...
	public function index($pageNumber=null)
	{
		...
		$data['param'] = str_replace('https://'.$_SERVER['HTTP_HOST'].'/post/','',$actuaLlink);
	
		if($data['param']=='https://'.$_SERVER['HTTP_HOST'].'/post'){
			$data['param'] = '';
		} else {
	...
	public function getAllPost($pageNumber=null)
	{
		...
		$allPost = getAllPost($data['pageNumber'],$data['q'])->data;
		...
		foreach($allPost as $post){ 
			...
			<tr>
			  ...
			  <td class="text-wrap" style="word-wrap: break-word;min-width: 160px;max-width: 160px;">
					<h6 class="mb-0 text-sm text-wrap">'. $post->attributes->title .'</h6>
			  </td>
	...
	public function getPageCount($pageNumber=null)
	{
		...
		$pageCount = getAllPost($pageNumber,$q)->meta->pagination->pageCount;
